...
public class UTObsoleteModsHandler
{
    public static List<String> obsoleteModsMessage()
    {
        ...
        if (Loader.isModLoaded("betterburning") && (UTConfig.TWEAKS_ENTITIES.utBBArrowsToggle || UTConfig.TWEAKS_ENTITIES.utBBCookedToggle || UTConfig.TWEAKS_ENTITIES.utBBExtinguishToggle || UTConfig.TWEAKS_ENTITIES.utBBOverlayToggle || UTConfig.TWEAKS_ENTITIES.utBBSpreadingToggle)) messages.add("Better Burning");
        ...
        if (Loader.isModLoaded("blockoverlayfix") && UTConfig.BUGFIXES_BLOCKS.utBlockOverlayToggle) messages.add("Block Overlay Fix");
        ...
        if (Loader.isModLoaded("chunkgenlimit") && UTConfig.TWEAKS_WORLD.utChunkGenLimitToggle) messages.add("Chunk Generation Limiter");
        if (Loader.isModLoaded("classiccombat") && UTConfig.TWEAKS_ITEMS.utAttackCooldownToggle) messages.add("Classic Combat");
        if (Loader.isModLoaded("collisiondamage") && UTConfig.TWEAKS_ENTITIES.utCollisionDamageToggle) messages.add("Collision Damage");
        ...
        if (Loader.isModLoaded("ding") && UTConfig.TWEAKS_MISC.utLoadSoundMode != 0) messages.add("Ding");
        if (Loader.isModLoaded("drawerfps") && UTConfig.MOD_INTEGRATION.utSDRenderRange > 0) messages.add("DrawerFPS");
        if (Loader.isModLoaded("deuf") && UTConfig.BUGFIXES_ENTITIES.utEntityUUIDToggle) messages.add("Duplicate Entity UUID Fix (DEUF)");
        if (Loader.isModLoaded("easybreeding") && UTConfig.TWEAKS_ENTITIES.utEasyBreedingToggle) messages.add("Easy Breeding");
        ...
        if (Loader.isModLoaded("mendingfix") && UTConfig.TWEAKS_ITEMS.utMendingToggle) messages.add("Mending Fix");
        if (Loader.isModLoaded("nanfix") && UTConfig.BUGFIXES_ENTITIES.utEntityNaNToggle) messages.add("NaN Entity Health Fix");
        if (Loader.isModLoaded("nobounce") && UTConfig.MOD_INTEGRATION.utTCStableThaumometerToggle) messages.add("Stable Thaumometer");
        ...
        if (Loader.isModLoaded("overpowered_mending") && UTConfig.TWEAKS_ITEMS.utMendingOPToggle) messages.add("Overpowered Mending");
        ...
        if (Loader.isModLoaded("smooth-scrolling-everywhere") && UTConfig.TWEAKS_MISC.utSmoothScrollingToggle) messages.add("Smooth Scrolling Everywhere");
        if (Loader.isModLoaded("stepupfix") && UTConfig.TWEAKS_ENTITIES.utAutoJumpToggle) messages.add("StepupFixer");
        if (Loader.isModLoaded("stg") && UTConfig.TWEAKS_MISC.utSwingThroughGrassToggle) messages.add("SwingThroughGrass");
        ...
        if (Loader.isModLoaded("unloader") && UTConfig.TWEAKS_WORLD.utUnloaderToggle) messages.add("Unloader");
